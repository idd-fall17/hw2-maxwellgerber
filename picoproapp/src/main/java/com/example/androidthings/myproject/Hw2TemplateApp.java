...
public class Hw2TemplateApp extends SimplePicoPro {
    ...
    private void  writeTextToScreen() {
        ...
        BackgroundColorSpan redSpan = new BackgroundColorSpan(Color.BLUE);
        ForegroundColorSpan fgSpan = new ForegroundColorSpan(Color.WHITE);
        Spannable sb = new SpannableString(screentext + tail);
        println("writing to screen: " + screentext + tail);
        sb.setSpan(redSpan, screentext.length(), screentext.length() + tail.length(), Spanned.SPAN_INCLUSIVE_INCLUSIVE);
        sb.setSpan(fgSpan, screentext.length(), screentext.length() + tail.length(), Spanned.SPAN_INCLUSIVE_INCLUSIVE);
        ed.setText(sb);
        ed.setSelection(screentext.length());
    }
